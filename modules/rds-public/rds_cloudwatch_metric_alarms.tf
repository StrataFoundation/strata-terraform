...
resource "aws_cloudwatch_metric_alarm" "cpu_utilization" {
  ...
  dimensions = {
    DBInstanceIdentifier = aws_db_instance.public_monitoring_rds.identifier
  }
}
...
resource "aws_cloudwatch_metric_alarm" "cpu_utilization_rr" {
  ...
  dimensions = {
    DBInstanceIdentifier = aws_db_instance.public_monitoring_rds_read_replica[0].identifier
  }
}
...
resource "aws_cloudwatch_metric_alarm" "maximum_used_transaction_ids" {
  alarm_name          = "${var.env}-${var.stage} - Monitoring RDS - Max Use Transaction IDs"
...
resource "aws_cloudwatch_metric_alarm" "maximum_used_transaction_ids_rr" {
  ...
  alarm_name          = "${var.env}-${var.stage} - Monitoring RDS Read Replica - Max Use Transaction IDs"
  alarm_description   = "Nearing a possible critical transaction ID wraparound."
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = "2"
  metric_name         = "MaximumUsedTransactionIDs"
